...
const GitHubOAuthCallback = () => {
    ...
    const handleOAuthCallback = async () => {
      try {
        ...
        if (error) {
          setStatus('error');
          setMessage(
            errorDescription || 'GitHub authorization was denied or failed.'
          );
        ...
        // Complete OAuth flow by saving to database
        setMessage('Completing GitHub authorization...');
        await githubService.completeOAuth(resultId);
  ...
  const getStatusIcon = () => {
    switch (status) {
      case 'processing':
        return (
          <LoaderIcon className="h-12 w-12 text-indigo-600 animate-spin" />
        );
      case 'success':
